...
void Optimizer::MMGlobalBundleAdjustemnt(Map* pMap, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    BundleAdjustment(vpKFs,vpMPs,nIterations,pbStopFlag, nLoopKF, bRobust);
}  
                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        vSE3->setId(pKF->mnId);
        vSE3->setFixed(pKF->mnId==0 && !pKF->isNotFixed());
...
void Optimizer::MMOptimizeEssentialGraph(Map* pMap, KeyFrame* pLoopKF, KeyFrame* pCurKF,
                                       const LoopClosing::KeyFrameAndPose &NonCorrectedSim3,
                                       const LoopClosing::KeyFrameAndPose &CorrectedSim3,
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    vector<MapPoint*> vpMPs1 = pMap->GetAllMapPoints();
    ...
    if(pMap->isAttached())
    {
        ...
        for(std::vector<Map*>::iterator it = vpAttachedMaps.begin(), itend = vpAttachedMaps.end(); it != itend; it++)
        {
            ...
            vector<KeyFrame*> vpAttachedKFs = pmMap->GetAllKeyFrames();
            vector<MapPoint*> vpAttachedMPs = pmMap->GetAllMapPoints();
            vpKFs1.insert(vpKFs1.end(), vpAttachedKFs.begin(), vpAttachedKFs.end());
            vpMPs1.insert(vpMPs1.end(), vpAttachedMPs.begin(), vpAttachedMPs.end());
        }
    }  
    

    const vector<KeyFrame*> vpKFs = vpKFs1;
    const vector<MapPoint*> vpMPs = vpMPs1;
