        ...
        mbDeactivateLocalizationMode(false),mbUseMMapping(bUseMMapping), mnId(nId)
{
    ...
    cout << endl <<
    "ORBSLAMM Copyright (C) 2016-2017 Hayyan Daoud, University of Malaya." << endl <<
...
//    mpMMapper->SetVocabulary(mpVocabulary);
    ...
    mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpMap, mpMMapper, mpMapSerializer, mpKeyFrameDatabase, strSettingsFile, mSensor, mbUseMMapping);
        ...
        mbDeactivateLocalizationMode(false),mbUseMMapping(bUseMMapping), mnId(nId)
{
    ...
    cout << "Vocabulary loaded in " << duration << " seconds!"<< endl << endl;
...
void System::SetMultiMapper(MultiMapper* pMMapper)
{
    mpMMapper = pMMapper;
    ...
    mpMMapper->SetTracker(mpMap, mpTracker);
    mpMMapper->SetLocalMapper(mpMap, mpLocalMapper);
    mpMMapper->SetLoopCloser(mpMap, mpLoopCloser);
    
    mpTracker->SetMMapper(mpMMapper);
}
...
bool System::IsUsingMMaps()
{
    return mbUseMMapping;
}
...
void System::Shutdown()
{
    ...
    while(!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished()  ||
          !mpViewer->isFinished()      || mpLoopCloser->isRunningGBA() || !mpMMapper->isFinished())
    {
        usleep(5000);
    }
...
void System::SaveMultipleMapsTrajectories(const string& filename) 
{
    //mpMMapper->AddMap(mpMap, mpKeyFrameDatabase);
    mpMMapper->SaveTrajectory(filename);
    
    //Save Keyframes and MapPoints
    //mpMapSerializer->SaveMap(mpMap, ".");
}

System::MapStatus System::SaveMap(const string& filename)
{
    cout << "\n Saving map to " << filename << " ..." << endl;
    mpMapSerializer->SaveMap(mpMap, filename);
    return MAP_OK;
